...
class Group(models.Model):
    ...
    def __str__(self):
        ...
        return f"{self.name} ({lang}) | {branch_name} | {self.chat_id}"
...
class Script(models.Model):
    ...
    def _get_target_group_ids(self):
        ...
        prefetched_branches = prefetched.get("branches")
        ...
        if prefetched_branches is not None and prefetched_groups is not None:
            ...
            group_ids.update(group.id for group in prefetched_groups)
            group_ids.discard(None)
            return group_ids

        branch_group_ids = self.branches.filter(is_active=True).values_list("groups__id", flat=True)
        selected_group_ids = self.groups.values_list("id", flat=True)
        group_ids = set(branch_group_ids).union(selected_group_ids)
        group_ids.discard(None)
        return group_ids

    def get_target_groups(self):
        group_ids = self._get_target_group_ids()
        return Group.objects.filter(id__in=group_ids).select_related("branch")
